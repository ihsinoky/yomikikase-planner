...
- [x] 年度ごとのデータ削除機能
  - [x] アンケート・回答・確定日程・年度別プロフィールの削除
  - [x] 削除前のエクスポート（バックアップ）機能
  - [x] アクティブアンケート年度の誤削除防止
- [x] 完全削除の方針を文書化（`docs/data-deletion-policy.md`）
- [x] 管理者引き継ぎガイド（`docs/admin-handover.md`）
- [x] 簡易運用マニュアルの作成（`docs/operations-manual.md`）

2026-04-10 完了。年度単位のデータエクスポート・削除 API を整備。
完全削除ポリシー、管理者引き継ぎガイド、運用マニュアルを作成。
