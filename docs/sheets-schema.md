...
## 6. ConfirmedEvents タブ

アンケート結果をもとに管理者が確定した読み聞かせ日程を保持します。

### 列定義

| 列名 | データ型 | 必須 | 説明 | 例 |
|------|---------|------|------|-----|
| eventId | string | ✓ | 確定日程の一意ID | `evt_001` |
| surveyId | string | | 元のアンケートID（任意） | `survey_001` |
| surveyDateId | string | | 元の候補日ID（任意） | `date_001` |
| fiscalYear | string | ✓ | 年度 | `2025` |
| eventDate | datetime | ✓ | 確定日時 | `2025-01-15 10:00:00` |
| targetGrade | string | ✓ | 対象学年 | `年少`, `年中`, `年長`, `全学年` |
| className | string | | 対象クラス（任意） | `さくら組` |
| confirmedAt | datetime | ✓ | 確定登録日時 | `2025-01-10 09:00:00` |
| notes | string | | 備考 | `園庭使用予定のため室内` |

### サンプルデータ

```csv
eventId,surveyId,surveyDateId,fiscalYear,eventDate,targetGrade,className,confirmedAt,notes
evt_001,survey_001,date_001,2025,2025-01-15 10:00:00,年少,,2025-01-10 09:00:00,
evt_002,survey_001,date_002,2025,2025-01-16 10:00:00,年中,,2025-01-10 09:00:00,園庭使用予定のため室内
```

---

## 7. EventParticipants タブ

確定日程に紐付く参加者を保持します。1つの確定日程に複数の参加者が紐付きます。

### 列定義

| 列名 | データ型 | 必須 | 説明 | 例 |
|------|---------|------|------|-----|
| participantId | string | ✓ | 参加者レコードの一意ID | `par_001` |
| eventId | string | ✓ | 確定日程ID（外部キー） | `evt_001` |
| lineUserId | string | ✓ | 参加者のLINE ユーザーID | `U1234567890abcdef` |
| role | string | | 役割（任意） | `reader` |
| confirmedAt | datetime | ✓ | 登録日時 | `2025-01-10 09:00:00` |
| notes | string | | 備考 | `午後から参加` |
...
```csv
participantId,eventId,lineUserId,role,confirmedAt,notes
par_001,evt_001,U1234567890abcdef,reader,2025-01-10 09:00:00,
par_002,evt_001,U2345678901bcdefg,reader,2025-01-10 09:00:00,午後から参加
par_003,evt_002,U3456789012cdefgh,reader,2025-01-10 09:00:00,
```
...
ConfirmedEvents (1) ←―→ (N) EventParticipants
  - 1つの確定日程に複数の参加者

Surveys (1) ←―→ (N) ConfirmedEvents
  - 1つのアンケートから複数の確定日程（任意）
