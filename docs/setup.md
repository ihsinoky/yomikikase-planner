...
**Cloudflare Pages で本番運用する場合（推奨）**は、こちらのクイックスタートガイドをご利用ください：

👉 **[Cloudflare 環境セットアップ - クイックスタート](cloudflare-quickstart.md)** 👈

このドキュメントは、Spreadsheet / GAS / LINE ミニアプリの詳細な設定手順を説明しています。
Cloudflare 固有の設定（Pages Functions、環境変数など）については、上記のクイックスタートガイドを参照してください。
...
- [4. 設定値の登録](#4-設定値の登録)
- [4.1. Milestone 3 の追加設定](#41-milestone-3-の追加設定)
- [5. 動作確認](#5-動作確認)
...
## 4.1. Milestone 3 の追加設定

Milestone 3 の LIFF ミニアプリを動かすには、Sprint 1 の設定に加えて以下が必要です。

### Cloudflare Pages 環境変数

- `GAS_BASE_URL`: GAS Web App の URL
- `GAS_API_KEY`: GAS の `API_KEY` と一致する値
- `LIFF_ID`: 本番または検証で使う LIFF ID
- `LINE_LOGIN_CHANNEL_ID`: LINE ID トークン検証に使うチャネル ID

### Spreadsheet 側の準備

- `Config.activeSurveyId` を設定する
- `Surveys` に対象アンケートを登録し、`status=active` にする
- `SurveyDates` に対象アンケートの候補日を登録する

### LINE Developers 側の確認

- LIFF の Endpoint URL が Cloudflare Pages の URL と一致している
- LIFF 側で `openid` scope を有効にし、ID トークンが取得できる
